...
const FIELDS = [
    ...
    'WorkOrder_Line_Item__c.Resource__r.FirstName',
    'WorkOrder_Line_Item__c.Resource__r.LastName',
];

export default class StaffingEditSKULWC extends NavigationMixin(LightningElement) {
    ...
    wiredLineItem({ error, data }) {
        ...
        if (data) {
            ...
            this.lineItemData = {
                ...
                resourceName: (data.fields.Resource__r.value?.fields?.FirstName?.value || '') + ' ' + (data.fields.Resource__r.value?.fields?.LastName?.value || '')
            };
    ...
    handleSave() {
        ...
        if (this.lineItemData.rate <= 0) {
            this.showToast('Error', 'Rate must be greater than zero', 'error');
        }
        this.isLoading = true;
