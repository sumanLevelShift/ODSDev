...
import fetchLookupData from '@salesforce/apex/HierarchyCustomLookupLwcController.fetchLookupData';
import fetchDefaultRecord from '@salesforce/apex/HierarchyCustomLookupLwcController.fetchDefaultRecord';

const DELAY = 300; // dealy apex callout timing in miliseconds  

export default class HierarchyCustomLookupLwc extends LightningElement {
    // public properties with initial default values 
    @api placeholder = '';
    @api typeLookup = 'createBy';
    @api sObjectApiName = 'Hierarchy_Lookup__c';
    @api defaultRecordId = '';
    @api disabled=false;
    // private properties 
    ...
    connectedCallback() {
        if (this.defaultRecordId != '') {
          /*  fetchDefaultRecord({ recordId: '$defaultRecordId', sObjectApiName: '$sObjectApiName' })
                .then((result) => {
                    if (result != null) {
    ...
    @wire(fetchLookupData, { searchKey: '$searchKey', sObjectApiName: '$sObjectApiName', typeLookup: '$typeLookup' })
    searchResult(value) {
        const { data, error } = value; // destructure the provisioned value
