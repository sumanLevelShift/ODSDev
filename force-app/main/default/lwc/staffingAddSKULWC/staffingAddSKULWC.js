...
import createStaffingWorkOrderLineItems from '@salesforce/apex/BC_WorkOrderLineItemLWCController.createStaffingWorkOrderLineItems';
...
export default class StaffingAddSKULWC extends NavigationMixin(LightningElement) {
    @api workOrderId = '';
    @track unitOfMeasure = '';
    @api skuResult = false;
    ...
    objectInfo({ data, error }) {
        if (data) {
            ...
            this.recordTypeId = Object.keys(recordTypes).find(
                rtId => recordTypes[rtId].name === 'Staffing'
            );
        }
        if (error) {
    ...
    handleResourceLookupUpdate(event) {
        const rowId = parseInt(event.target.dataset.rowId);
        const selectedRecord = event.detail.selectedRecord;
        var resourceId;
    ...
    handleSave() {
        ...
        createStaffingWorkOrderLineItems({
            lineItems: lineItems,
    ...
    validateRows() {
        ...
        for (let i = 0; i < this.skuRows.length; i++) {
            ...
            if (!row.resourceId) {
                isValid = false;
                errorMessage = `Please select an Resource for row ${i + 1}`;
                break;
            }
            if (!row.uOM || row.uOM === '') {
                isValid = false;
