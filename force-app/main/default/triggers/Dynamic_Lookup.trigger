...
// changed if condition to send error message only if the opportunity is being assigned to lead recruiter -- abinaya 16.apr.12
    ...
    if(myopportunity.AssignedUser__c == null && myopportunity.AssignedUser__r.Profile.Name == 'Lead Recruiter'|| myopportunity.AssignedUser__r.Profile.Name == 'LR Chatter Only User')
    {
      ...
      for(AggregateResult ar1 : val)
       {
        ...
        System.Debug('CurrUserCnt :'+ CurrUserCnt );
          ...
          if(Integer.valueOf(ar1.get('cnt'))>=10)
             {
                LRcount = 100;  
...
// LIST<User> UserList = [Select u.Id from User u where u.UserRole.Name = 'Lead Recruiter' and u.UserRole.ParentRoleId != null    and u.ManagerId!=null  and u.Id!=:myopportunity.AssignedUser__c]; //and u.ManagerId = :myopportunity.ManagerId__c
 LIST<User> UserList = [Select u.Id from User u where (u.Profile.Name ='Lead Recruiter' or u.Profile.Name ='LR Chatter Only User') and u.UserRole.ParentRoleId != null    and u.ManagerId!=null  and u.Id!=:myopportunity.AssignedUser__c and u.IsActive=true]; //and u.ManagerId = :myopportunity.ManagerId__c
